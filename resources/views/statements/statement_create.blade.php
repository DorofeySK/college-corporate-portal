...
@section('left_part')
<form id="create_statement" action="{{ route('statements.store') }}" method="POST" class="w-full p-8 flex flex-col space-y-4 items-center">
    @csrf
    <select v-model="payment_name" onchange="setFullName();" size="5" id="payment_id" name="payment_id" class="w-full p-2 border-b border-black">
    ...
    <select v-model="payment_type" size="5" id="paymentdetail_id" name="paymentdetail_id" class="w-full p-2 border-b border-black">
    ...
    <input v-model="score" type="number" id="amount_id" name="amount" placeholder="Баллы" max="" min="0" class="w-full p-2 border-b border-black">
    <select v-model="docs" multiple size="5" name="doc_ids[]" class="w-full p-2 border-b border-black">
    ...
    <select v-model="checker" size="5" name="checker_login" class="w-full p-2 border-b border-black">
    ...
    <input :disabled="is_valide" type="submit" value="Добавить запись о выплате" class="p-2 border-b border-black hover:bg-black hover:text-white disabled:opacity-50">
</form>
<script>
...
<script type="module">
    createApp({
        ...
        computed: {
            is_valide: function() {
                return this.payment_type == '' || this.payment_name == '' || this.score == '' || this.docs == '' || this.checker == '';
            }
        }
    }).mount('#create_statement');
</script>
