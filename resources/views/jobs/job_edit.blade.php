    ...
    <form action="{{ route('jobs.update', ['id' => $job->id]) }}" method="POST" class="w-full p-8 flex flex-col space-y-4 items-center">
        ...
        <select multiple size="8" name="roles[]" class="w-full p-2 border-b border-black">
            ...
            @foreach ($roles as $key => $value)
                <option @if(in_array($key, $job->getRoles())) selected @endif value={{ $key }}>{{ $value }}</option>
            @endforeach
        </select>
        <input type="submit" value="Обновить должность" class="p-2 border-b border-black hover:bg-black hover:text-white">
    </form>
</div>
